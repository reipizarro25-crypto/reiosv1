...
#paintCanvas,#cmdOutput,#chatOutput { border:2px solid currentColor; image-rendering: pixelated; display:block; }
#cmdOutput, #chatOutput { width:100%; height:calc(100% - 25px); overflow-y:auto; font-size:10px; padding:2px; }
...
<div id="desktop">
  ...
  <div class="icon" style="top:80vh; left:5vw;" draggable="true" data-app="Chat"><img src="https://img.icons8.com/ios-filled/50/00ff00/chat.png"/><div>CHAT</div></div>
  <div class="icon" style="top:95vh; left:5vw;" draggable="true" data-app="Settings"><img src="https://img.icons8.com/ios-filled/50/00ff00/settings.png"/><div>SETTINGS</div></div>
</div>
...
<div id="startMenu">
  ...
  <button onclick="openWindow('Chat')">Chat</button>
  ...
  switch(title){
    ...
    case 'Paint': 
      return `<div style="margin-bottom:5px;">
                ...
                <button onclick="setBrush(2)">Small</button>
                <button onclick="setBrush(5)">Medium</button>
                <button onclick="setBrush(10)">Large</button>
    ...
    case 'Chat':
      return `<div id="chatOutput"></div>
              <input type="text" id="chatInput" placeholder="Type message..." style="width:80%">
...
function resizeAppContent(title){
  ...
  if(title==='Paint'){
    ...
    canvas.height = win.clientHeight-100;
  ...
  if(title==='Chat'){
    const chatOut = win.querySelector('#chatOutput');
    if(chatOut) chatOut.style.height=(win.clientHeight-50)+'px';
  }
}
...
// Paint
let paintCanvas, paintCtx, painting=false, currentTool='brush';
function initPaint(win){
  ...
  function draw(e){
    ...
    paintCtx.lineWidth=parseInt(document.getElementById('brushSize')?.value||5);
    ...
    if(currentTool==='brush'){
      ...
      bucketFill(Math.floor(x),Math.floor(y), document.getElementById('paintColor').value);
      painting=false;
    }
  }
...
function setBrush(size){ document.getElementById('brushSize').value=size; currentTool='brush'; }
function setTool(tool){ currentTool=tool; }

// Simple bucket fill (pixel-based)
function bucketFill(startX,startY,fillColor){
  ...
  const getColor=(x,y)=>[data[px(x,y)],data[px(x,y)+1],data[px(x,y)+2],data[px(x,y)+3]];
  const setColor=(x,y,r,g,b,a)=>{ const i=px(x,y); data[i]=r; data[i+1]=g; data[i+2]=b; data[i+3]=a; };
  const target=getColor(startX,startY);
  const colorHex=fillColor.match(/\w\w/g).map(h=>parseInt(h,16));
  if(target[0]===colorHex[0] && target[1]===colorHex[1] && target[2]===colorHex[2]) return;
  ...
  while(stack.length>0){
    ...
    if(c[0]===target[0] && c[1]===target[1] && c[2]===target[2]){
      setColor(x,y,colorHex[0],colorHex[1],colorHex[2],255);
...
// Chat
let chatMessages=[];
function initChat(win){ resizeAppContent('Chat'); updateChat(); }
function sendChat(){
  const input=document.getElementById('chatInput');
  if(input.value.trim()==='') return;
  chatMessages.push({user:'You', msg:input.value});
  chatMessages.push({user:'Bot', msg:'Echo: '+input.value}); // simulate response
  input.value='';
  updateChat();
}
function updateChat(){
  ...
  if(chatOut){
    chatOut.innerHTML='';
    chatMessages.forEach(m=>chatOut.innerHTML+='<div><strong>'+m.user+':</strong> '+m.msg+'</div>');
    chatOut.scrollTop=chatOut.scrollHeight;
  }
}

// Settings
function changeUIColor(c){ currentColor=c; document.querySelectorAll('*').forEach(el=>{const s=getComputedStyle(el); if(s.color===currentColor) el.style.color=c; if(s.borderColor===currentColor) el.style.borderColor=c;}); }
